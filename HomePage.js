...
function HomePage(props) {
 ...
 // Retrieve the NHS number from the query string
 const urlParams = new URLSearchParams(location.search);
 const nhsNumber = urlParams.get('nhs');
  ...
  useEffect(() => {
    ...
    var url = `http://localhost:4000/Homepage.php?nhs=${nhsNumber}`;
    $.get(url, (data) => {
      setData(data);
    }).fail((error) => {
  ...
  return (
    <Page>
      ...
      <p>Your NHS Number is {nhsNumber}</p>
      <div>
